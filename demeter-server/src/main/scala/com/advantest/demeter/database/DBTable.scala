...
trait DBTable:
  ...
  // Defines a type alias for the table schema, which must be a subclass of Table and DBTableSchema.
  protected type TableType <: Table[TableRowData] & DBTableSchema
  ...
  protected def createTableIfNotExists(): Unit =
    ...
    db.run(table.schema.createIfNotExists).onComplete:
      case Success(_) =>
        ...
        system.log.info(s"Table created successfully, schema: ${table.schema.createStatements}")
      case Failure(error) =>
        // Log an error message if the table creation fails.
        system.log.error(error.toString)
   ...
   * @return
   */
  def tableQuery: TableQuery[TableType] = table
  ...
  def insert(data: TableRowData): Future[TableRowData] =
    ...
    db.run(transaction).map:
      case Some(value) => value.asInstanceOf[TableRowData]
